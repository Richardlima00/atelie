...
def homepage(request):
    ...
    apresenta= Apresentacao.objects.filter(ativo=True)
    context = {"banners": banners, "apresenta": apresenta }
...
def checkout(request):
    if request.method == 'POST':
        if not request.user.is_authenticated:
            ...
            # Criação do cliente anônimo ou recuperação a partir do cookie
            id_sessao = request.COOKIES.get("id_sessao", get_random_string(32))
            response = redirect('finalizar_pedido')
            response.set_cookie('id_sessao', id_sessao)
            cliente, criado = Cliente.objects.get_or_create(id_sessao=id_sessao, email=email)

            # Criação do endereço
            Endereco.objects.create(cliente=cliente, rua=rua, numero=numero,complemento=complemento, cep=cep, cidade=cidade, estado=estado)
            response.set_cookie('endereco', endereco.id)
...
def finalizar_pedido(request, id_pedido):
    if request.method == 'POST':
        ...
        if not 'endereco' in dados:
            # if endereco is not provided, we will create one for non-logged users
            if not request.user.is_authenticated:
                rua = dados.get('rua')
            ...
            if not erro:
                ...
                if clientes:
                    ...
                    if pedido.cliente is None:
                        pedido.cliente = Cliente(email=email)
                    else:
